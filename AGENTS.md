...
| `src/app/api/plan/route.ts` | POST `{request, preferences, learned, currentPlan?, history?}` → **streamed** `text/plain` body: the structured-output JSON document (`PlanResponseSchema` shape) piped to the client delta-by-delta. With `currentPlan` set it refines instead of regenerating, keeping untouched block ids stable. Pre-stream failures return JSON `{error}`; mid-stream failures abort the body (client's final parse fails). |
| `src/lib/streaming.ts` | Client-side progressive parsing: `repairAndParse()` (close open strings/brackets on a JSON fragment) + `parsePartialPlanResponse()` (extract the partial reply and only the blocks/legs/lodging that already zod-validate). |
...
- Model: `claude-sonnet-4-6` (via `PLANNER_MODEL` — change there only). Chosen for cost; bump to `claude-opus-4-8` if itinerary quality needs it. Never guess model ids — there is no "sonnet 4.7"; unknown ids 404.
